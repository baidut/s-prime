...
classdef RomaDataset < vvDataset
    properties (Constant)
        ...
        % To refine our analysis the database is also divided into three sub-classes:
        %  1.curved road images,
        %  2.adverse lighting conditions and eroded markings,
        %  3.straight road under good lighting conditions and with good quality markings.
    ...
    methods (Access = public)
            ...
            try
                ...
                for iSit = 1:numel(sit)
                    for jSce = 1:numel(sceVal)
                        [f,tmp] = roma.loadMov(sit{iSit},sceVal{jSce}); % column vector
                        n = numel(f);
                ...
                roma.data = table(name, situation, scenario, filename);
                
            catch
                error('Fail in loading roma dataset. Please check your path.');
            end
        ...
        function [imageList, name] = loadMov(roma, situation, scenario)
            folder = fullfile(roma.path,situation);
            movFile = fullfile(folder,scenario);
            name = roma.loadmov(movFile);
            imageList = strcat(folder,filesep,name);
        end
    end
    
    methods (Static)
        function algos = eval_road(algos, situationIndex, fileIndex)
            ...
            for algo = algos
                for iSitu = situationIndex % 1:n
                    ...
                    imgDataset.rawImgFile = imgFiles;
                    imgDataset.gtImgFile = this.ROMA.roadAreaGt(imgFiles);
                    benchmark(iSitu) = Benchmark(evalAlgo, imgDataset);
                end
                
            end
        end
        
        function info = fileInfo(file)
        ...
        function gtFiles = markAreaGt(files)
            func = @(f) [f(1:end-12) 'R' f(end-11:end-4) '.pgm'];
            switch(class(files))
                case 'cell'
