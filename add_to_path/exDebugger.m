classdef exDebugger
    ...
    methods (Static)
        function savegcf(filename, saveEps)
            handle = gcf;
            if saveEps
    ...
    methods (Access = public)
        ...
        function implotd(obj, level, varargin)
        ...
        function imdumpd(obj, level, varargin)
            % imdump debug version
            ...
            for i = 1:numel(varargin)
                param = varargin{i};
                filename = inputname(i+nargin-numel(varargin));
                ...
                if 1 == length(param) && ishandle(param) %% handle
                    ...
                    if isfield(obj.dumper,'nameFunc')
                        filename = obj.dumper.nameFunc(handle);
                    end
